...
def lambda_handler(event, context):
    ...
    api_key_dict = get_secret(
        'operations-engineering-gandi-api-key', 'us-east-1')
    ...
    certificate_id = get_certificate_id_from_domain_name(
        certificate_list, domain_name)

    dry_run_error = certificate_regeneration(
        certificate_id, csr, dcv_method, api_key, True)
    ...
    if dry_run_error:
        ...
        real_run_error = certificate_regeneration(
            certificate_id, csr, dcv_method, api_key)
    ...
    pending_certificate_id = get_pending_certificate_with_retry(
        certificate_list, domain_name)

    dcv_subdomain, dcv_value = get_dcv_details(
        pending_certificate_id, api_key, dcv_method)
...
def certificate_regeneration(certificate_id: str, csr: str, dcv_method: str, api_key: str, dry_run: bool = False):
    ...
    try:
        conn.request(
            "P", f"/v5/certificate/issued-certs/{certificate_id}", body=body, headers=headers)
...
def get_pending_certificate_with_retry(certificate_list, domain_name, max_attempts=5, delay=10):
    for attempt in range(max_attempts):
        try:
            pending_certificate_id = get_certificate_id_from_domain_name(
                certificate_list, domain_name, True)
            if pending_certificate_id is not None:
                return pending_certificate_id
            else:
                print(
                    f"No pending certificate found on attempt {attempt + 1}. Retrying in {delay} seconds...")

                time.sleep(delay)
        except Exception as e:
            print(
                f"An error occurred on attempt {attempt + 1}: {str(e)}. Retrying in {delay} seconds...")

            time.sleep(delay)

    raise Exception(
        "Failed to retrieve a pending certificate after maximum attempts.")
...
def get_dcv_details(certificate_id, api_key, dcv_method):
    ...
    try:
        conn.request(
            "POST", f"/v5/certificate/issued-certs/{certificate_id}/dcv_params", body=body, headers=headers)
    except Exception as e:
        print(f"An error occured when retrieving the dcv details: {e}")

    response = json.loads(conn.getresponse().read().decode('utf-8'))

    dcv_subdomain, dcv_value = response.get(
        'raw_messages')[0][0], response.get('raw_messages')[0][1],
...
def get_certificate_list_with_http(api_key):
    ...
    conn.request(
        "GET", "/v5/certificate/issued-certs?per_page=1000", headers=headers)
...
def get_secret(secret_name, region_name):
    ...
    try:
        ...
        raise Exception(
            f"Error retrieving secret: {e.response['Error']['Message']}") from e

    else:
        if 'SecretString' in get_secret_value_response:
            secret_dict = json.loads(get_secret_value_response['SecretString'])
        else:
            secret_dict = json.loads(base64.b64decode(
